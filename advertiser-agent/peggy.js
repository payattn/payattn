...
class PeggyAgent {
   ...
   */
  async start() {
    console.log('[OK][OK] Peggy starting up...');
    console.log('=========================================\n');
    ...
    try {
      ...
      if (this.advertiser.wallet_pubkey !== this.escrow.getPublicKey().toBase58()) {
        console.warn(`[OK][OK]  WARNING: Wallet mismatch!`);
      ...
      console.log(`\n[OK][OK][OK] Peggy initialized successfully!`);
      ...
      while (true) {
        try {
          ...
          console.error('[OK][OK][OK] Error in main loop:', error.message);
      ...
      console.error('[OK][OK][OK] Peggy startup failed:', error.message);
      process.exit(1);
    }
  }
  ...
  async processOffers() {
    ...
    console.log(`\n[${timestamp}] [OK][OK] Found ${offers.length} pending offer(s)\n`);
    ...
    for (const offer of offers) {
      ...
      try {
        ...
        console.error(`[OK][OK][OK] Failed to handle offer ${offer.offer_id}:`, error.message);
  ...
  async handleOffer(offer) {
    ...
    console.log(`[OK][OK] Evaluating offer ${offer.offer_id}`);
    ...
    if (decision.decision === 'reject') {
      console.log(`\n[OK][OK][OK] Offer ${offer.offer_id} rejected\n`);
      return;
    }
    
    // Accept offer (get x402 response)
    console.log(`\n[OK][OK] Accepting offer ${offer.offer_id}...`);
    const x402 = await this.api.acceptOffer(offer.offer_id);
    
    console.log(`\n[OK][OK] Received HTTP 402 Payment Required:`);
